...
// Mapeo de controladores a sus tags ordenados
const tagMapping = {
...
Object.entries(tagMapping).forEach(([filename, newTag]) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    // Buscar y reemplazar el tag existente
    ...
    if (content.match(tagRegex)) {
      ...
      console.log(`✅ Actualizado: ${filename} -> ${newTag}`);
    } else {
      console.log(`⚠️  No se encontró tag en: ${filename}`);
    }
  } else {
...
console.log('\n🎯 Orden correcto de endpoints en Swagger:');
